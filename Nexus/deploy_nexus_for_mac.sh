...
            echo -e "[$(get_timestamp)] ${GREEN}检测到配置文件中的 Node ID：${CURRENT_NODE_ID}${NC}"
...
                echo -e "[$(get_timestamp)] ${GREEN}已更新 Node ID: $NODE_ID_TO_USE${NC}"
            else
                NODE_ID_TO_USE="$CURRENT_NODE_ID"
            fi
        else
            echo -e "[$(get_timestamp)] ${YELLOW}未检测到有效 Node ID，请输入新的 Node ID。${NC}"
            read -rp "请输入新的 Node ID: " NODE_ID_TO_USE
            mkdir -p "$HOME/.nexus"
            echo "{\"node_id\": \"${NODE_ID_TO_USE}\"}" > "$CONFIG_PATH"
            echo -e "[$(get_timestamp)] ${GREEN}已写入 Node ID: $NODE_ID_TO_USE 到 $CONFIG_PATH${NC}"
        fi
    else
        echo -e "[$(get_timestamp)] ${YELLOW}未找到配置文件 $CONFIG_PATH，请输入 Node ID。${NC}"
        read -rp "请输入新的 Node ID: " NODE_ID_TO_USE
        mkdir -p "$HOME/.nexus"
        echo "{\"node_id\": \"${NODE_ID_TO_USE}\"}" > "$CONFIG_PATH"
        echo -e "[$(get_timestamp)] ${GREEN}已写入 Node ID: $NODE_ID_TO_USE 到 $CONFIG_PATH${NC}"
