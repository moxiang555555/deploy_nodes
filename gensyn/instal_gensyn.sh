...
verify_password() {
    ...
    while [[ $attempt -le $max_attempts ]]; do
        ...
        # 这里设置你的实际密码，建议使用强密码
        # 密码哈希值：bushu001 -> 5e884898da28047151d0e56f8dc6292773603d0d6aabbdd62a11ef721d1542d8
        # 密码哈希值：bushu002 -> 7c4a8d09ca3762af61e59520943dc26494f8941b
        local password1_hash="5e884898da28047151d0e56f8dc6292773603d0d6aabbdd62a11ef721d1542d8"
        local password2_hash="7c4a8d09ca3762af61e59520943dc26494f8941b"
        
        # 计算输入密码的哈希值
        local input_hash=$(echo -n "$password" | openssl dgst -sha256 | cut -d' ' -f2)
        
        if [[ "$input_hash" == "$password1_hash" ]]; then
            ...
            return 0
        elif [[ "$input_hash" == "$password2_hash" ]]; then
